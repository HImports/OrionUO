...
void COrion::ClearUnusedTextures()
{
	...
	int counts[5] =
	{
		MAX_ART_OBJECT_REMOVED_BY_GAGE_COLLECTOR,
		MAX_ART_OBJECT_REMOVED_BY_GAGE_COLLECTOR,
		MAX_GUMP_OBJECT_REMOVED_BY_GAGE_COLLECTOR,
		MAX_ART_OBJECT_REMOVED_BY_GAGE_COLLECTOR,
		100
	};
	...
	for (deque<CIndexSound*>::iterator it = m_UsedSoundList.begin(); it != m_UsedSoundList.end();)
	{
		...
		if (obj->LastAccessTime < g_Ticks)
		{
			...
			if (++count >= MAX_SOUND_OBJECT_REMOVED_BY_GAGE_COLLECTOR)
				break;
		}
		else
			++it;
...
void COrion::DrawLandTexture(CLandObject *land, ushort color, const int &x, const int &y)
{
	...
	else
	{
		...
		if (!g_GrayedPixels && color)
		{
			glUniform1i(g_ShaderDrawMode, SDM_LAND_COLORED);
			g_ColorManager.SendColorsToShader(color);
		}
		else
			glUniform1i(g_ShaderDrawMode, SDM_LAND);
